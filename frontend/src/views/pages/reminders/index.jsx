...
import { useTaskComplete, useTaskDelete, useTaskSave, useTaskUpdate } from "../../../api-hooks/task";
...
import { useReminderList } from "../../../api-hooks/reminder";
...
const Reminder = () => {
  const STATUS_OPTIONS = commonObj?.constants?.Taskstatus || [];
  ...
  // ---------- Query State (search) ----------
  const [search, setSearch] = useQueryStates({ key: parseAsString.withDefault(null)},{ urlKeys: { key: "q" }, ...DEFAULT_NUQS_CONFIG });
  ...
  const {data:taskRes, isFetching:isLoading, isError, error, refetch, refetchWithQuery} = useReminderList({qData:{page:1, limit:20, ...search, ...filters}});
  const reminderList = taskRes?.data;
  ...
  const statCardData = [
    {icon: <ClockCircleOutlined />, title: "Total Reminders", value: 34, change: "+12 this week"},
  ...
  const formatDate = (dateString) => {
    ...
    if (isToday) `Today at ${timeString}`;
         ...
         title={
          <Space align="center">
            ...
            <Space direction="vertical" size={0}>
              <Title level={4} style={{margin:0}}>Reminder Management</Title>
              <Text type="secondary" style={{fontSize:12}}>Manage reminders</Text>
            </Space>
          </Space>
         }
         extra={
          <Space align="center">
            ...
            {<Button type="primary" icon={<PlusOutlined />} onClick={()=>handleReminderModal('other')} size="middle" style={{borderRadius:999, paddingInline:18}}>Create Reminder</Button>}
          </Space>
         }
        >
          {/* Stats grid (4 cards) */}
          <Row gutter={[12, 12]} style={{marginBottom:16}}> 
            {statCardData?.map(((item, idx) => (<Col key={idx} xs={12} md={6}><StatCard icon={item.icon} title={item.title} value={item.value} change={item.change}/></Col>)))}
          </Row>
            ...
            <Flex vertical gap={12} style={{ marginTop: 16 }}>
              {reminderList?.length === 0 ? (
                <Card bordered style={{textAlign: "center", padding: 24, borderRadius: 12, marginTop: 12}}>
                  ...
                  <Title level={4} style={{ marginBottom: 4 }}>No Reminders found</Title>
                  <Text type="secondary"> No reminders match your current filter selection.</Text>
                  <div style={{ marginTop: 16 }}>
                    <Button type="primary" onClick={handleAdd}> Add Your First Reminder </Button>
                  </div>
                </Card>
              ) : (
                <div>
                <Space vertical style={{width:'100%'}}>
                  {reminderList?.map((reminder) => {
                    ...
                    const isCompleted = String((reminder?.isCompleted || "")).toLowerCase();
                  ...
                  return (
                    <Card key={reminder?.id} size="small" bordered hoverable styles={{body:{padding:14}}} style={{borderRadius: 12, boxShadow: "0 4px 12px rgba(15,23,42,0.08)", borderColor: overdue ? "#ffccc7" : undefined}}>
                        ...
                        <Col flex="auto">
                            <Space align="flex-start" size={12}>
                                ...
                                <Flex align="center" justify="center" style={{width: 32, height: 32, borderRadius: 10, backgroundColor: isCompleted ? "rgba(82,196,26,0.12)" : "rgba(107,114,128,0.12)", color: isCompleted ? "#52c41a" : "#6b7280"}}>
                                  {reminder?.lead ? <UserOutlined /> : <ClockCircleOutlined />}
                                </Flex>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <Flex wrap align="center" gap={8} style={{ marginBottom: 4 }}>
                                  <Text strong style={{ fontSize: 14, textDecoration: isCompleted ? "line-through" : "none", opacity: isCompleted ? 0.75 : 1}}>{reminder?.title}</Text>
                                  <Tag color={reminder?.entityType === "call" ? "blue" : "purple"} style={{ borderRadius: 999, fontSize: 11 }}> {reminder?.entityType?.toUpperCase() || "OTHER"}</Tag>
                                </Flex>
                              ...
                              {reminder?.lead && (
                                ...
                                <Space size={10} wrap>
                                  ...
                                  <Text strong style={{ fontSize: 12 }}> {reminder.lead.fullName}</Text>
                                  <Text type="secondary" style={{ fontSize: 12 }}>{reminder.lead.phone}</Text>
                                  {reminder.lead.stage && ( <Tag color="geekblue" style={{ borderRadius: 999, fontSize: 11 }}>{reminder.lead.stage.replace("_", " ")}</Tag>)}
                                </Space>
                                </div>
                              )}
                          ...
                          <Col flex="none">
                            <Flex vertical align="end" gap={6}>
                              {/* PRIORITY */}
                              <Tag color={getPriorityColor(reminder?.priority)} style={{ borderRadius: 999, fontSize: 11 }}> {reminder?.priority || "Low"} </Tag>
                              {/* STATUS */}
                              <Tag color={getStatusTagColor(reminder?.status)} style={{ borderRadius: 999, fontSize: 11 }}> {getStatusLabel(reminder?.status)}</Tag>
                              {/* ACTIONS */}
                              <Space size={4} style={{ marginTop: 4 }}>
                                {acc.editAccess && ( <Tooltip title="Edit">
                                  <Button size="small" type="text" icon={<EditOutlined />} onClick={()=>{handleReminderModal(); setData(reminder)}} />
                                </Tooltip> )
                                }

                                {(!isCompleted && acc.completeAccess ) &&
                                  reminder?.assignedTo?.id === commonObj?.id && (
                                  <Tooltip title="Mark as complete">
                                    <Button size="small" type="text" icon={<CheckCircleOutlined />} style={{ color: "#52c41a" }} onClick={() => completeTask(reminder?.id)}/>
                                  </Tooltip>
                                )}

                                {acc.deleteAccess && <Tooltip title="Delete">
                                  <Button size="small" type="text" danger icon={<DeleteOutlined />} onClick={() => deleteTask(reminder?.id)} />
                                  </Tooltip>
                                } 
                              </Space>
                            </Flex>
          ...
          <Modal open={isEditModalOpen} onCancel={handleModalClose} destroyOnHidden footer={null} centered width={900} title={null} styles={{body:{padding:0, background:'linear-gradient(135deg, #f3f6ff 0%, #ffffff 35%, #fdf5ff 100%)'}}} >
            <Flex justify="space-between" align="center" style={{padding: 20, paddingBottom: 12, borderBottom: "1px solid #f0f0f0", gap: 8}}>
              <Space align="center">
                ...
                <Space direction="vertical" size={0}>
                  <Title level={5} style={{ margin: 0 }}>{selectedId ? "Edit Reminder" : "Create Reminder"}</Title>
            ...
            <div style={{ padding: 18, paddingTop: 6 }}>
              ...
              <DataForm reminderCheck={reminderCheck} setReminderCheck={setReminderCheck} id={selectedId} onSave={()=>handleModalClose(true)} reminderModal={reminderModal} handleReminderModal={handleReminderModal} initialData={data} leadId={leadId}/>
            </div>
          </Modal>

          {/* Reminder Modal */}
          <ReminderModal data={data} open={reminderModal} onClose={handleReminderModal} id={data?.id} entityType={entityType}/>
  ...
  return (
    <Form onFinish={() => {refetch()}} style={{ width: "100%" }}>
      <Row gutter={8} wrap align="middle">
        <Col span={20}>
          <Input placeholder="Search by name" value={search.key ?? ""} prefix={<SearchOutlined style={{ color: "#bfbfbf" }} />} onChange={(e) => setSearch({ key: e.target.value })} allowClear
...
function DataForm({ id, onSave, initialData, leadId, handleReminderModal, reminderCheck, setReminderCheck }) {
  ...
  const TASK_TYPE_OPTIONS = commonObj?.constants?.Tasktype || [];
  const STATUS_OPTIONS = commonObj?.constants?.Taskstatus || [];
  const PRIORITY_OPTIONS = commonObj?.constants?.Taskpriority || [];
          ...
          <Row gutter={12}>
            ...
            <Col xs={24} md={10}>
              <Form.Item name="type" label="Task Type" rules={[{ required: true, message: "Please select task type" }]}>
                <Select placeholder="Select type" options={TASK_TYPE_OPTIONS?.map(t => ({ label: t, value: t }))} />
              </Form.Item>
            </Col>
          ...
          <Row gutter={12}>
            ...
            <Col xs={24} md={8}>
              <Form.Item name="priority" label="Priority">
                <Select placeholder="Select priority" options={PRIORITY_OPTIONS?.map((v) => ({label: v, value: v}))} />
              </Form.Item>
            </Col>
            ...
            {isEdit && (
              <Col xs={24} md={8}>
                <Form.Item name="status" label="Status">
                  <Select placeholder="Select status"  options={STATUS_OPTIONS?.map((v) => ({label: v, value: v,}))} />
                </Form.Item>
              </Col>
            )}
          </Row>
